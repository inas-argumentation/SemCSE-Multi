...
from medical_embeddings.models import MistralTransformationModel
from medical_embeddings.auxiliary import load_medical_dataset, relative_path
from medical_embeddings.train_llama_decoders import create_unified_precomputed_embeddings, get_training_data
...
MISTRAL_DECODER_SAVE_DIR = relative_path("../data/saved_models/medical/mistral_decoders")
...
def train_decoder_for_type(embedding_type, llm, processor, train_data, val_data):
    ...
    for epoch in range(100):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_no_improve = 0
            save_path = os.path.join(MISTRAL_DECODER_SAVE_DIR, f"decoder_{embedding_type}.pth")
...
def train_all_mistral_decoders():
    os.makedirs(MISTRAL_DECODER_SAVE_DIR, exist_ok=True)
    ...
    embedding_types_to_train = prompt_identifiers
...
def test_decoder_generation(embedding_type, num_samples=5):
    print(f"\n--- Testing decoder for '{embedding_type}' ---")

    decoder_path = os.path.join(MISTRAL_DECODER_SAVE_DIR, f"decoder_{embedding_type}.pth")
    ...
    with torch.no_grad():
        for sample_id, sample_data in test_samples:
            ...
            embedding = torch.tensor(sample_data["embeddings"][embedding_type], dtype=torch.float32).unsqueeze(0).to(device)
            llm_dtype = next(llm.parameters()).dtype
