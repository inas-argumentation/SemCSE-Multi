...
from transformers import AutoTokenizer, AutoModelForCausalLM, AutoModelForImageTextToText, AutoProcessor, BitsAndBytesConfig
from medical_embeddings.models import MistralTransformationModel, LlamaTransformationModel, UnifiedEmbeddingModel
from medical_embeddings.auxiliary import load_medical_dataset, relative_path
...
UNIFIED_MODEL_PATH = relative_path(f"../data/saved_models/medical/unified_embedding_model.pkl")
LLAMA_DECODER_DIR = relative_path("../data/saved_models/medical/llama_decoders")
MISTRAL_DECODER_DIR = relative_path("../data/saved_models/medical/mistral_decoders")
    ...
    with torch.no_grad():
        for sample_id, sample_data in tqdm(eval_data.items(), desc="Collecting embeddings"):
            ...
            for etype in embedding_types:
                ...
                embedding = unified_embeddings[etype]
...
def load_evaluation_data():
    ...
    return {x: dataset[x] for x in list(dataset.keys())[:500]}
...
def evaluate_all_llama_decoders():
    ...
    results = {etype: {'losses': [], 'perplexities': []} for etype in prompt_identifiers}
    ...
    with torch.no_grad():
        for sample_id, sample_data in tqdm(eval_data.items(), desc="Evaluating Llama Samples"):
            ...
            for etype in prompt_identifiers:
                summaries = sample_data.get(etype, [])
...
def evaluate_llama_decoders_shuffled():
    ...
    embeddings_by_type = {etype: [] for etype in prompt_identifiers}
    summaries_by_type = {etype: [] for etype in prompt_identifiers}
    ...
    for etype in prompt_identifiers:
        ...
        with torch.no_grad():
            for embedding, shuffled_summary_list in tqdm(zip(embeddings_by_type[etype], shuffled_summary_lists), desc=f"Evaluating shuffled {etype} embeddings..."):
                sample_losses = []
                sample_perplexities = []
...
def evaluate_tsne_roundtrip(model_type="llama", limit_per_aspect=None):
    ...
    samples_by_type = _collect_samples_by_type(unified_model, unified_tokenizer, eval_data, prompt_identifiers, device)
...
def evaluate_models_unconditioned():
    ...
    eval_data = load_evaluation_data()
    embedding_types = prompt_identifiers
