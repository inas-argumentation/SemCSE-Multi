...
def load_individual_embedding_models():
    ...
    for p in prompt_identifiers:
        model_path = relative_path(f"../data/saved_models/medical/embedding_model_{p}.pkl")
        proj_path = relative_path(f"../data/saved_models/medical/embedding_proj_{p}.pkl")
...
def create_precomputed_embeddings_dataset():
    precomputed_path = relative_path(f"../data/precomputed_embeddings_medical.pkl")
    ...
    for sample_id, sample_data in tqdm(list(dataset.items())[500:], desc="Processing samples"):
...
def train_unified_model():
    ...
    for epoch in range(1, 100):
        ...
        if eval_loss < best_loss:
            best_loss = eval_loss
            torch.save(model.state_dict(), relative_path(f"../data/saved_models/medical/unified_embedding_model.pkl"))
