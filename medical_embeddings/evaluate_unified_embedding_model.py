...
from medical_embeddings.models import UnifiedEmbeddingModel
from medical_embeddings.generate_aspect_specific_summaries import prompts

prompt_identifiers = list(prompts.keys())

PAIRS_DIR = relative_path("../data/llm_pairwise_assessments/medical")
SAVED_MODELS_DIR = relative_path("../data/saved_models/medical")
MISTRAL_SUMMARIES_JSON = relative_path("../data/mistral_medical_summaries.json")
BATCH_SIZE = 16
...
def load_summarizing_sentences():
    ...
    for idx_str, sample_data in list(data.items())[:500]:
        idx = int(idx_str)
...
def print_results_table(results_matrix, title):
    aspects = sorted(results_matrix[list(results_matrix.keys())[0]].keys())
    pred_aspects = sorted(results_matrix.keys())
...
def main():
    ...
    model = UnifiedEmbeddingModel(encoder_checkpoint, prompt_identifiers, proj_dimension).to(DEVICE)
    tokenizer = AutoTokenizer.from_pretrained(encoder_checkpoint)

    try:
        state = torch.load(os.path.join(SAVED_MODELS_DIR, "unified_embedding_model.pkl"), map_location=DEVICE)
...
def evaluate_baseline_models():
    ...
    try:
        ...
        print("[WARN] sentence-transformers library not available, skipping SentenceTransformer models")
    ...
    baseline_defs = [
        {"name": "Clinical-ModernBERT", "hf_id": "Simonlee711/Clinical_ModernBERT", "loader": "transformers", "distance": "cosine"},
    ...
    if SENTENCE_TRANSFORMERS_AVAILABLE:
        baseline_defs.append(
            {"name": "MedEmbed", "hf_id": "abhinand/MedEmbed-base-v0.1", "loader": "sentence_transformers",
             "distance": "cosine"}
        )
    ...
    for gt_aspect in aspects:
        ...
        for bdef in baseline_defs:
            ...
            try:
                embeddings = _compute_embeddings_for_baseline(hf_id, sample_texts, loader=loader,
                                                              adapter_name=adapter_name, custom_weights=custom_weights)
            except Exception as e:
            ...
            try:
                spearman_corr = compute_mean_spearman_correlation(
                    pred_similarity_matrix, gt_similarity_matrix)
                top10_acc = compute_top_k_accuracy_matrix(pred_similarity_matrix, gt_similarity_matrix, k=10)
                ...
                print(f"[WARN] Failed to evaluate baseline {name} vs gt={gt_aspect}: {e}")
                results_spearman[name][gt_aspect] = None
                results_top10[name][gt_aspect] = None

    print_results_table(results_spearman, "Baseline Models - Mean Spearman Correlation")
    print_results_table(results_top10, "Baseline Models - Top-10 Accuracy")
