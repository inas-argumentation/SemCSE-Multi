...
os.makedirs(relative_path("../data/saved_models/medical"))

print("Analyzing training data:")
for p in prompt_identifiers:
    print(f"{p}: {len([x for x in list(dataset.values())[500:] if len([y for y in itertools.chain.from_iterable([x[c] for c in x.keys() if p in c])
...
def train_embedding_model(prompt_code):
    ...
    available_data = [[x for x in itertools.chain.from_iterable([x[c] for c in x.keys() if prompt_code in c]) if "not applicable" not in x.lower()]
                      for x in list(dataset.values())[500:]]
    ...
    for epoch in range(1, 500):
        ...
        for batch_idx in tqdm(range(1000), desc=f"Training (epoch {epoch})", file=sys.stdout):
            ...
            next_batch = get_train_batch(SENTENCE_BATCH_SIZE)
        ...
        if val_score > max_val_score:
            ...
            torch.save(model.state_dict(), relative_path(f"../data/saved_models/medical/embedding_model_{prompt_code}.pkl"))
            torch.save(projection.state_dict(), relative_path(f"../data/saved_models/medical/embedding_proj_{prompt_code}.pkl"))
...
def train_all_individual_prompt_embedding_models():
    for p in prompt_identifiers:
        if not os.path.exists(relative_path(f"../data/saved_models/medical/embedding_model_{p}.pkl")):
