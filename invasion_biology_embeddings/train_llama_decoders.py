...
from invasion_biology_embeddings.models import UnifiedEmbeddingModel, LlamaTransformationModel
...
LLAMA_DECODER_SAVE_DIR = relative_path("../data/saved_models/invasion_biology/llama_decoders")
...
def create_unified_precomputed_embeddings(original_dataset, device):
    ...
    precomputed_data = {}
    with torch.no_grad():
        for sample_id, sample_data in tqdm(original_dataset["unlabeled"].items(), desc="Generating Unified Embeddings"):
...
def get_training_data(embedding_type, unified_precomputed_data):
    ...
    for sample_id, precomputed_sample in unified_precomputed_data.items():
        if ((embedding_type in precomputed_sample["embeddings"] or embedding_type.split("_")[0] in precomputed_sample["embeddings"])
                and embedding_type in precomputed_sample["summaries"]):
            embedding = precomputed_sample["embeddings"][embedding_type.split("_")[0]]
...
def train_all_llama_decoders():
    ...
    for embedding_type in individual_prompt_identifiers:
        save_path = os.path.join(LLAMA_DECODER_SAVE_DIR, f"llama_decoder_{embedding_type}.pth")
...
def test_all_llama_decoders(num_samples=3):
    embedding_types_to_test = individual_prompt_identifiers
